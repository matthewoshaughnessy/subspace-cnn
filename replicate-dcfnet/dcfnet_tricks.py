...
lr_def = 0.005
if len(sys.argv) > 5:
    lr_def = float(sys.argv[5])
print("Default learning rate is: ",lr_def)
momentum_def = 0.9
if len(sys.argv) > 6:
    momentum_def = float(sys.argv[6])
...
if useTricks:
    transform = transforms.Compose(
        [transforms.ToTensor(),
         transforms.Normalize((0.0, 0.0, 0.0), (1.0, 1.0, 1.0))]) # transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
else:
    transform = transforms.Compose(
        [transforms.ToTensor(),
...
class Net(nn.Module):
    def __init__(self):
        ...
        if useTricks:
            nn.init.kaiming_normal(self.conv1.weight, mode='fan_in')
            nn.init.kaiming_normal(self.conv2.weight, mode='fan_in')
            nn.init.kaiming_normal(self.conv3.weight, mode='fan_in')
            nn.init.kaiming_normal(self.fc1.weight, mode='fan_in')
            nn.init.kaiming_normal(self.fc2.weight, mode='fan_in')
