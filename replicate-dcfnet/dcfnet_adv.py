...
elif basisType == 'random':
    X1 = np.random.randn(H1*W1,H1*W1)
    X2 = np.random.randn(H2*W2,H2*W2)
    X3 = np.random.randn(H3*W3,H3*W3)
    basis1, _ = scipy.linalg.qr(X1)
    basis2, _ = scipy.linalg.qr(X2)
    basis3, _ = scipy.linalg.qr(X3)
