...
for epoch in range(nEpochs):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        if torch.cuda.is_available():
            inputs, labels = Variable(inputs.cuda(device_id=0)), Variable(labels.cuda(device_id=0))
        else:
        ...
        if torch.cuda.is_available():
            outputs = net(inputs.cuda(device_id=0))
        else:
            outputs = net(inputs)
        ...
        if i % 10 == 9:    # print every 2000 mini-batches
            ...
            if subspaceProject:
                if torch.cuda.is_available():
                    ...
                    net.conv1.weight.data = (torch.from_numpy(w1p)).type(torch.FloatTensor).cuda(device_id=0)
                    net.conv2.weight.data = (torch.from_numpy(w2p)).type(torch.FloatTensor).cuda(device_id=0)
                else:
