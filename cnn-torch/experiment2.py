...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr  8 12:08:38 2018

@author: Rakshith
"""

##################################################################
### Experiment 1: larger CNN  ####################################
##################################################################
...
if reducedData:
    print('Reduced training set ON')
    print('Reduced training set ON', file=open(outputFile,'a'))
else:
    print('Noisy training set OFF')
    print('Noisy training set OFF', file=open(outputFile,'a'))      
...
if reducedData:
    ...
    features = np.reshape(train_data_batch,(50000,3,32,32)).astype('uint8')
    features = np.transpose(features,(0,2,3,1))
    labels = unpickle(labels_filename)
...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 16 * kern_size * kern_size)
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        x = self.fc3(x)
        return x
